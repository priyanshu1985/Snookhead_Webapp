...
const ActiveSession = () => {
  ...
  }, [activeType, computedCategories, selectedCategory]);
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (passedSession) {
          ...
          if (passedSession.tableid) {
            try {
              ...
              console.warn("Failed to fetch table info", e);
            }
          }
          setLoading(false);
          return;
        ...
        if (foundSession) {
          ...
          if (
            ...
            try {
              const savedCart =
                typeof foundSession.food_orders === "string"
    ...
    fetchSession();
  }, [tableId, sessionId, passedSession, initialCart]);
  ...
  const addToCart = async (item) => {
    setCart((prev) => {
      ...
      if (session) {
        const sessionId = session.active_id || session.activeid;
        activeTablesAPI
          .update(sessionId, { food_orders: newCart })
          .catch((e) => {
    ...
    if (session) {
      try {
        ...
        const orderPayload = {
          ...
          personName:
            session.customer_name || session.customername || "Table Customer",
          order_source: "active_session",
          status: "pending",
          payment_status: "pending",
          paymentMethod: "offline", // Default to cash for table orders
        ...
        ordersAPI
          .create(orderPayload)
          .catch((e) => console.error("Kitchen order trigger failed", e));
      } catch (err) {
  ...
  const handleGenerateBillClick = () => {
    ...
    const durationMins =
      session?.duration_minutes || session?.durationminutes || 0;
    const isEarlyExit =
      isTimerMode && session && durationMins > elapsedMinutes + 1;
  ...
  const showBillConfirmation = (overrideDuration = null) => {
    ...
    const isPaid = finalGrandTotal <= advancePayment && advancePayment > 0;
    const actionText = isPaid ? "End Session" : "Generate Bill";
    const remainingAmount = Math.max(0, finalGrandTotal - advancePayment);
    ...
    const confirmMessage = `
        <div style="text-align: left">
           ...
           <div style="margin-top: 10px; padding: 10px; background: #f9f9f9; border-radius: 8px;">
              ...
              ${advancePayment > 0 ? `<p style="color: ${isPaid ? "#10b981" : "#f59e0b"}; font-weight: 600;"><strong>${isPaid ? "✓ Advance Paid (Full)" : "⚠ Advance Paid (Partial)"}:</strong> -₹${advancePayment.toFixed(2)}</p>` : ""}
              <p style="font-size: 1.1em; margin-top: 6px; color: ${isPaid ? "#10b981" : "#1a1a2e"}; font-weight: ${isPaid ? "700" : "600"};"><strong>To Pay:</strong> ₹${remainingAmount.toFixed(2)}</p>
              ${isPaid ? '<p style="color: #10b981; font-size: 13px; margin-top: 8px; font-weight: 600;">✓ Bill fully paid. Session will be ended and moved to history.</p>' : ""}
           </div>
        </div>
      `;
  ...
  const processGenerateBill = async (overrideDuration = null) => {
    ...
    try {
      ...
      // UPDATE: billingAPI.create now handles stopping the session on the backend.
      // We do NOT need to call stop() again, as it will fail with "Session is not active".
      // await activeTablesAPI.stop({ active_id: sessionId, skip_bill: true });

      // Pass the new bill ID to the billing page for highlighting
      const newBillId =
        billResponse?.data?.id || billResponse?.id || billResponse?.bill?.id;

      // Decide which tab to open: if paid, go to history
      // Recalculate total if override duration is provided
      let finalTableCost = tableCost;
      let finalGrandTotal = grandTotal;

      if (overrideDuration !== null) {
        finalTableCost = isFrameMode
          ? frameCount * frameCharge
          : overrideDuration * pricePerMin;
        finalGrandTotal = finalTableCost + cartTotal;
      }
      ...
      navigate("/billing", {
        state: {
          billGenerated: true,
          newBillId,
          activeTab: isPaid ? "history" : "active",
        },
      });
    } catch (err) {
  ...
  useEffect(() => {
    const fetchLinkedReservation = async () => {
      ...
      // PRIORITY 1: Check if session already has advance_payment field (from database)
      if (
        session.advance_payment !== undefined &&
        session.advance_payment !== null
      ) {
        setAdvancePayment(Number(session.advance_payment));
      }
      ...
      try {
        ...
        if (match) {
          if (match.notes) {
            // ONLY extract payment from notes if not already set from session
            if (!session.advance_payment) {
              // Regex to match any PAID tag: [PAID_X: amount]
              const paymentMatch = match.notes.match(
                /\[PAID_(?:ADVANCE|HALF|CASH|UPI|WALLET):\s*(\d+(\.\d+)?)\]/,
              );
          ...
          <div className="session-header">
            <div className="header-left">
              ...
              <div className="header-title-group">
                ...
                {/* Customer Info - Show if available from queue transfer */}
                {session?.customer_name &&
                  session.customer_name !== "Table Customer" && (
          ...
          <div className="session-content">
            ...
            <div className="session-left-column">
              <div className="booking-panel">
                {/* Customer Info Section - Show queue transfer data */}
                {session?.customer_name &&
                  session.customer_name !== "Table Customer" && (
                    <div
                    ...
                    >
                      {/* Customer Name and Payment Badge in One Row */}
                      <div
                        style={{
                          display: "flex",
                          alignItems: "center",
                ...
                <div className="panel-section highlight-section text-center">
                  ...
                  {/* Main Timer Display */}
                  <div
                    className={`timer-display-unified ${isTimerMode && getTimerColor()}`}
                  >
                    {isFrameMode
                      ? frameCount
                      : isStopwatchMode
                  ...
                  <div className="session-controls-unified">
                    <button
                      className={`control-btn-unified ${isPaused ? "paused" : ""}`}
                      onClick={togglePause}
                    >
                    ...
                    {isFrameMode && (
                      <div
                        className="frame-controls-unified"
                          ...
                          onClick={() => {
                            ...
                            showConfirm(
                              ...
                              async () => {
                                try {
                                  setGenerating(true);
                                  const newCount = Math.max(0, frameCount - 1);
                                  // Optimistic Update
                                  setSession((prev) => ({
                                    ...prev,
                                    frame_count: newCount,
                      ...
                      >
                        ...
                        >
                          ...
                          {session?.food_orders && (
                            ...
                            >
                              (Pre-selected from queue)
                            </span>
                          )}
                        </label>
                        {cart.map((item) => (
                          <div
                            className="bill-row item-row"
                            key={item.id}
                            style={{
                              fontSize: "13px",
                              color: "#666",
                      ...
                      <button
                        ...
                        style={
                          grandTotal <= advancePayment && grandTotal > 0
                            ? {
                                background:
                                  "linear-gradient(135deg, #10b981 0%, #059669 100%)", // Green for Paid
          ...
          <div className="pricing-summary mobile-pricing">
            <div className="price-row">
              {isFrameMode ? (
                <>
                  <span>
                    Frame Charges ({frameCount} frame
                    {frameCount !== 1 ? "s" : ""})
                  </span>
      ...
      {showTimeModal && (
        ...
        >
          ...
          >
            ...
            >
              Enter minutes to extend session
            </p>
            ...
            >
              <button
                onClick={() =>
                  setManualTime((prev) => Math.max(5, Number(prev) - 5))
                }
                style={{
                  width: "44px",
                  height: "44px",
              ...
              <div style={{ position: "relative" }}>
                ...
                <div
                  style={{
                    ...
                    height: "3px",
                    background:
                      "linear-gradient(90deg, #f08626 0%, #ff6b00 100%)",
                    borderRadius: "2px",
                  }}
                ></div>
                <div
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background:
                    "linear-gradient(135deg, #f08626 0%, #ff6b00 100%)",
                  color: "white",
                  fontWeight: "600",
                  cursor: "pointer",
        ...
        >
          ...
          >
            ...
            >
              <button
                onClick={() =>
                  setFramesToAdd((prev) => Math.max(1, Number(prev) - 1))
                }
                style={{
                  width: "44px",
                  height: "44px",
      ...
      {showEarlyExitModal && earlyExitData && (
        ...
        >
          ...
          >
            ...
            >
              This session was booked for{" "}
              <strong>{earlyExitData.bookedDuration} mins</strong>,<br />
              but currently only{" "}
              <strong>{earlyExitData.actualDuration} mins</strong> have elapsed.
            </p>
            <p
              style={{
                color: "#333",
            ...
            >
              ...
              <button
                onClick={() => handleEarlyExitSelection("actual")}
                style={{
                  padding: "16px",
                  borderRadius: "12px",
              ...
              >
                <div>
                  ...
                  >
                    Actual Time ({earlyExitData.actualDuration}m)
                  </div>
                  <div style={{ fontSize: "12px", color: "#666" }}>
                    Pay only for time played
                  </div>
                </div>
                <div
                  style={{
              ...
              {/* Option 2: Full Time */}
              <button
                onClick={() => handleEarlyExitSelection("full")}
                style={{
                  padding: "16px",
                  borderRadius: "12px",
                  border: "1px solid #e0e0e0",
              ...
              >
                <div>
                  ...
                  >
                    Full Booking ({earlyExitData.bookedDuration}m)
                  </div>
                  <div style={{ fontSize: "12px", color: "#666" }}>
                    Pay for full booked session
                  </div>
                </div>
                <div
                  style={{ fontWeight: "700", color: "#333", fontSize: "16px" }}
                >
      ...
      {showCancelModal && (
        ...
        >
          ...
          >
            ...
            >
              Are you sure? Use this only for mistakes.
              <br />
              <strong>No bill will be generated</strong> and the table will be
